...
function scatterTemp () {
  ...
  const getMonth = stringDate => stringDate.split('-')[1]
  const getDay = stringDate => stringDate.split('-')[2]
  ...
  const updateMax = () => {
    ...
    d3.csv(`csv/day-by-day/${ciudad}-diarias.csv`).then(data => {
      ...
      layer
        ...
        .on('mouseover', d => {
          ...
          tooltip
            .style('opacity', 1)
            .html(
              `<p class="tooltip-scatter-text">La temperatura máxima de ${ciudad} el ${d.day}.${d.month}.${d.year} fue de ${d.maxima}ºC<p/>`
            )
            .style(
              'left',
  ...
  // LOAD THE DATA
  const loadData = () => {
    d3.csv('csv/day-by-day/Albacete-diarias.csv').then(data => {
      dataz = data.filter(d => String(d.fecha).match(reValueDate))
